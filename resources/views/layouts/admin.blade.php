...
<body class="bg-slate-50 dark:bg-[#080c14] text-slate-900 dark:text-slate-100 min-h-screen transition-colors duration-300">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            @if(Auth::check() && strtolower(Auth::user()->role) !== 'superadmin' && Auth::user()->school && !Auth::user()->school->isSubscriptionActive())
                <div class="bg-red-600 text-white px-8 py-2.5 text-center text-[10px] font-black uppercase tracking-[0.2em] z-[60] shadow-lg">
                    <div class="max-w-screen-2xl mx-auto flex items-center justify-center gap-3">
                        ...
                        <span>Masa aktif Instansi habis. Akses saat ini: <span class="text-yellow-300 underline">Read-Only</span>.</span>
                        <a href="{{ route('subscription.index') }}" class="bg-white text-red-600 px-3 py-1 rounded-full hover:bg-yellow-300 transition-all font-bold">Perpanjang Sekarang →</a>
                    </div>
                </div>
            @endif
